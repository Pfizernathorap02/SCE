<script language="javascript">
    ...
    function showDocument(name) {
      window.open('<%=request.getContextPath()%>/evaluation/resources/pdf/'
				+ name, 'newDocWin',
				'height=720,width=616,resizable,scrollbars');
	}
</script>
...
<%
	...
	boolean isAdmin = user != null
			&& "SCE_Administrators".equalsIgnoreCase(user
					.getUserGroup())
			|| "SCE_OpsManager".equalsIgnoreCase(user.getUserGroup());
	
	//variable set is used in printBlankform.jsp page to hide/show form template
	boolean isAdministrator = user!=null
			&& "SCE_Administrators".equalsIgnoreCase(user
					.getUserGroup());
	//code end
	
	boolean isTrainingTeam = user != null
			&& "SCE_TrainingTeam".equalsIgnoreCase(user.getUserGroup());
	...
	boolean isGuestTrainer = user != null
			&& "SCE_GuestTrainer_NonMGR".equalsIgnoreCase(user
					.getUserGroup())
			|| "SCE_GuestTrainer_MGR".equalsIgnoreCase(user
					.getUserGroup());
	String UserLoginEmplid = user.getEmplId();
	System.out.println("UserLoginEmplid in navbar="+user.getEmplId());
	System.out.println("name="+user.getFirstName());
	
	
	//added by Manish for GuestTrainerManager
	boolean isGuestTrainerManager = user != null && "SCE_GuestTrainer_MGR".equalsIgnoreCase(user.getUserGroup());
	
	//LoggerHelper.logSystemDebug("User:" + user);    
	/*     System.out.println(isAdmin);
	 System.out.println(isTrainingTeam);
	 System.out.println(isGuestTrainer); */
	String admin = "";
...
<UL id=<%=(isAdmin || isTrainingTeam) ? "top_nav" : "top_nav1"%>>
	...
	<LI id=nav_search><s:url action="searchHome" var="schUrl"></s:url>
		<s:a href="%{schUrl}">Search</s:a></LI>
	...
	%>
	<LI id=nav_admin><s:url action="admin" var="adminUrl"></s:url>
		<s:a href="%{adminUrl}">Admin</s:a></LI>
	<%
		}
	%>
	...
	<LI id=nav_faq><s:url action="printblankform" var="printUrl"></s:url>
		<s:a href="%{printUrl}">Print blank form</s:a></LI>
