...
public class EvaluateAttendeeAction extends ActionSupport implements ServletRequestAware,ModelDriven  
{
	    ...
	    try{
		 ...
		 if(loginUserNtId!=null){
		 	...
		 	if(user!=null){
				...
				// System.out.println("user id = "+user.getId());//This line is for teting only
				if((user.getStatus()).equals("ACTIVE")){
					     ...
					     UserLegalConsent userLegalConsent=new UserLegalConsent();
	       				 userLegalConsent=sceManager.checkLegalConsentAcceptance(loginUserNtId);
	                     product=sceManager.getProductName(productCode);
	       				 
	       				 if(userLegalConsent==null){//USer has not yet accepted the Legal Consent
	                       ...
	                       req.setAttribute("evaluatorEmplId",user.getEmplId());
					                       
					       return "redirectToLc";
					  }
	                  else{ //userLegalConsent!=null => User has already accepted legal consent
					        req.setAttribute("linkName",linkName); 
	                        req.setAttribute("loginUserNtId",loginUserNtId);
	                        req.setAttribute("product",product);
	                        req.setAttribute("eventId",eventId);
	 ...
	 public String evaluateSCE()
	    {
		 HttpServletRequest req = getServletRequest();
	       ...
	       // System.out.println("SCE ID"+sceId);
	        ...
	        if(emplId !=null && activityId !=null && evaluatorEmplId!=null && empName!=null && evaluatorEmplName !=null && isFlag !=null && isSAdmin !=null)
	        {
	 ...
	 public String submitSCEEvaluate()
	    {
	        ...
	        /*String activityId = sceEvaluateForm.getActivityId();
	        String repId = sceEvaluateForm.getReprenstativeName();
	        String phaseNo= sceEvaluateForm.getPhaseNo();
	        String comment1 = sceEvaluateForm.getComment1();
	        String comment2 = sceEvaluateForm.getComment2();
	        String comment3 = sceEvaluateForm.getComment3();
	        String trackId= sceEvaluateForm.getTrackId();
	        String sceId=sceEvaluateForm.getSceId();*/
	        ...
	        if(phaseCount.intValue()==1)
	        {
	            ...
	            try {
					sceExistingcomment=sceManager.getExistingSCEComment(phaseNo,sceId,repId);
				} catch (SCEException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} catch (SQLException e) {
	       ...
	       /* if(enteredby1 !=null && !enteredby1.equalsIgnoreCase(form.getEvaluatorName()))
	        {
	            enteredby1=form.getEvaluatorName();
	        } */
	        String enteredby2 = req.getParameter("enteredBy2");
	       /*  if(enteredby2 !=null && !enteredby2.equalsIgnoreCase(form.getEvaluatorName()))
	        {
	            enteredby2=form.getEvaluatorName();
	        } */
	        String enteredby3=req.getParameter("enteredBy3");
	       /* if(enteredby3 !=null && !enteredby3.equalsIgnoreCase(form.getEvaluatorName()))
	        {
	            ...
	            if(comment1.equals("Comments")&&comment2.equals("Comments")&&comment3.equals("Comments"))
	                {
	                ...
	                {   
	                    ...
	                    {
	                        if( ((date1 == null || date1.equals("null")) &&  (enteredby1 == null ||enteredby1.equals("null"))) || (!comment1.equalsIgnoreCase(sceExistingcomment.getComment1())))
	                        {
	                    ...
	                    {
	                         if(((date2==null||date2.equals("null")) && (enteredby2==null||enteredby2.equals("null"))) || (!comment2.equalsIgnoreCase(sceExistingcomment.getComment2())))
	                        {
	                    ...
	                    else
	                    {
	                        ...
	                        if(((date3 == null || date3.equals("null")) && (enteredby3 == null || enteredby3.equals("null"))) || (!comment3.equalsIgnoreCase(sceExistingcomment.getComment3())))
	                        {
	                ...
	                if(phaseCount.intValue() == 1)
	                {
	                    ...
	                    try {
							sceManager.updateSCEComment(sceId,activityId,phaseNo,repId,comment1,comment2,comment3,enteredby1,enteredby2,enteredby3,date1,date2,date3,submitted_by,submitted_date);
						} catch (SCEException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						} catch (SQLException e) {
	                    ...
	                    session1.setAttribute("successmessage","Data updated successfully");
	                }
	                else
	                {
	                    try {
							sceManager.insertSCEComment(sceId,activityId,phaseNo,repId,comment1,comment2,comment3,enteredby1,enteredby2,enteredby3,date1,date2,date3,submitted_by,submitted_date);
						} catch (SCEException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						} catch (SQLException e) {
	...
	public String goToHistoryCheckTr()
	{
	    ...
	    try{
			...
			req.setAttribute("events", events);
	         ...
	         System.out.println("//Call method to get the SCE history object");//Call method to get the SCE history object
	         ...
	         userLegalConsent=sceManager.checkLegalConsentAcceptance(loginUserNtId);
	         user = sceManager.getUserByNTId(loginUserNtId);
	         System.out.println("After Legal Consent-------");
	         
	         if(sce!=null)
	         {//The history object is not null
	           // System.out.println("sce object not null - Length of History record [] sce = "+sce.length); 
	           
	           if(sce.length==1)
	           { //If there is only 1 history record
	...
	// System.out.println("phasedTraining"+phasedTraining);
	        ...
	        req.setAttribute("mode", SCEConstants.CREATE_MODE);
	            ...
	            if (phasedTraining) {
	                // P2L Phased Training
	                objSCE = sceManager.getLatestSCE(emplId, eventId, productCode, SCEConstants.ST_DRAFT);
	                if (objSCE == null) {
	                    objSCE = sceManager.getNewSCE(eventId, SCEConstants.EVENT_COURSE_ALL,product);
...
//        EvaluateForm evaluateForm =  new EvaluateForm();
        try{
       // Integer sceId = new Integer(Integer.parseInt(req.getParameter("sceid")));
	...
	public String evaluateTR()
    {
        ...
        if ("create".equalsIgnoreCase(action)) {
            if (!SCEUtils.isFieldNotNullAndComplete(emplId) || !SCEUtils.isFieldNotNullAndComplete(strEventId) || !SCEUtils.isFieldNotNullAndComplete(product) || !SCEUtils.isFieldNotNullAndComplete(productCode)) {
                return new String("failure");
            }
        }
        
        if ("view".equalsIgnoreCase(action)) {
            if (!SCEUtils.isFieldNotNullAndComplete(emplId) || !SCEUtils.isFieldNotNullAndComplete(strEventId) || !SCEUtils.isFieldNotNullAndComplete(productCode)) {
                return new String("failure");
            }
        }
        ...
        Integer rowNum=null;
        Double examScore = null;
      //  boolean phasedTraining = (eventId != null && eventId.intValue() >= 5 && eventId.intValue() <= 7);
        if(eventId != null)
        {
             rowNum= sceManager.isPhaseTraining(eventId);
        ...
        /*if (phasedTraining && !SCEUtils.isFieldNotNullAndComplete(strExamScore)) {
            return new String("failure");
        }*/
        // System.out.println("phasedTraining"+phasedTraining);
        ...
        if ("view".equalsIgnoreCase(action)) {
            ...
            // objSCE = sceManager.getLastFilledSCE(emplId, eventId, productCode);
            objSCE = sceManager.getLatestSCE(emplId, eventId, productCode, SCEConstants.ST_SUBMITTED);
            ...
            if (phasedTraining) {
                ...
                objSCE = sceManager.getLatestSCE(emplId, eventId, productCode, SCEConstants.ST_DRAFT);
                if (objSCE == null) {
                    objSCE = sceManager.getNewSCEPhase(eventId, SCEConstants.EVENT_COURSE_ALL);
                    objSCE.setStatus(SCEConstants.ST_NEW);                    
                } else {                                        
                    objSCE.setStatus(SCEConstants.ST_DRAFT);                    
                }
                ...
                objSCE = sceManager.getNewSCE(eventId, SCEConstants.EVENT_COURSE_ALL,objSCE.getProduct());
                objSCE.setSubmittedByEmplId(evaluatorEmplId);
            }
	...
	public String acceptLegalConsentTr(){
        ...
        try{
           ...
           /*if(req.getParameter("loginUserNtId")!=null){
                // System.out.println("Inside check for req. param - loginUserNtId");
                loginUserNtId = req.getParameter("loginUserNtId");
           }*/
	...
	private void setExamScore(SCE objSCE, Double examScore) {
        if (objSCE != null && examScore != null) {
            ...
            for (Iterator iter =  objSCE.getSceDetailList().iterator(); iter.hasNext();) {
                ...
                if (SCEConstants.QT_SCORE_FETCH.equalsIgnoreCase(objSCEDetail.getType())) {                                    
                    objSCEDetail.setAverageScore(new Double(evaluateExamScore(examScore)));
                    if (objSCE.getOverallScore() == null) {
                        objSCE.setOverallScore(new Double(objSCEDetail.getWeightedScore().doubleValue()));
                    } else {
                        objSCE.setOverallScore(new Double(objSCE.getOverallScore().doubleValue() + (objSCEDetail.getWeightedScore().doubleValue())));
                    }                    
                }                    
            } 
        }
	 ...
	 public String submitSCETRPhase()
	    {
		 ...
		 HttpServletRequest req = getServletRequest();
	        ...
	        if (SCEUtils.isFieldNotNullAndComplete(req.getParameter("overall_rating"))) {
	        ...
	        if (SCEUtils.isFieldNotNullAndComplete(req.getParameter("healthcare_compliant"))) {
	        ...
	        if (SCEConstants.ST_DRAFT.equalsIgnoreCase(status)) {
	            ...
	            if (SCEUtils.isFieldNotNullAndComplete(req.getParameter("overall_score"))) {
	                try {
	                    overall_score = new Double(req.getParameter("overall_score"));
	                }
	                catch (Exception e) {
	                }
	            }
	        ...
	        for (int i=0; i<objSCEDetailArr.length; i++) {
	            ...
	            /* P2L Fields */
	            if (SCEUtils.isFieldNotNullAndComplete(req.getParameter("question_score1_" + objSCEDetailArr[i].getQuestionId()))) {
	                question_score1 = new Double(req.getParameter("question_score1_" + objSCEDetailArr[i].getQuestionId()).trim());
	            }
	            if (SCEUtils.isFieldNotNullAndComplete(req.getParameter("question_score2_" + objSCEDetailArr[i].getQuestionId()))) {
	                question_score2 = new Double(req.getParameter("question_score2_" + objSCEDetailArr[i].getQuestionId()).trim());
	            }
	            if (SCEUtils.isFieldNotNullAndComplete(req.getParameter("question_score3_" + objSCEDetailArr[i].getQuestionId()))) {
	                question_score3 = new Double(req.getParameter("question_score3_" + objSCEDetailArr[i].getQuestionId()).trim());
	            }
	            /* END P2L Fields */
	        ...
	        //sceLogger.debug("EmplId:" + evaluateForm.getEvalEmplId() + " ProductCode:" + evaluateForm.getEvalProductCode() + " Product:" + evaluateForm.getEvalProduct());
	        req.setAttribute("evalEmplId",evaluateForm.getEvalEmplId());
	 ...
	 public String submitSCETR()
	    {
		 ...
		 HttpServletRequest req = getServletRequest();
	        ...
	        String overall_rating = null;
	        if (SCEUtils.isFieldNotNullAndComplete(req.getParameter("overall_rating_H"))) {
	        ...
	        try {
	            if(SCEUtils.isFieldNotNullAndComplete(evaluateForm.getEvalTrainingDate())){
	            trainingDate = (Date)formatter.parse(evaluateForm.getEvalTrainingDate());
	            }
	        }
	        catch (Exception e) {
	            //sceLogger.error("Training Date Parse:" + evaluateForm.getEvalTrainingDate());
	        }
	         /* End SCE Enhancement 2011*/
	        objSCE.setTemplateVersionId(templateVersionId);
	        objSCE.setPrecallRating(precall_rating);
