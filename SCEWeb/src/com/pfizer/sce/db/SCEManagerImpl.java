...
public class SCEManagerImpl {
	...
	static Logger log = LogManager.getLogger(SCEManagerImpl.class);
	...
	public TemplateVersion retrieveFirstEvalForm(String formStatus)
		...
		try {
			tv = sceControl.retrieveFirstEvalForm(formStatus);
		} catch (SQLException sqle) {
	...
	public TemplateVersion[] retrieveEvalForm(String formStatus)
		...
		TemplateVersion[] tv = null;
		try {
			tv = sceControl.retrieveEvalForm(formStatus);
		} catch (SQLException sqle) {
			// log.error("SQLEXCEPTION in retrieveFirstEvalForm", sqle);
			throw new SCEException("error.sce.database.communication", sqle);
		} catch (Exception e) {
	...
	public Attendee[] getAttendeesBySearchCriteria(Integer eventId,
			...
			String salesPositionTypeCd, String userGroup, String businessUnit)
			throws SCEException {
		try {
			...
			String strEventId = "";
			if (eventId != null) {
				strEventId = eventId.toString().equals(
						SCEConstants.ALL_EVENT_ID) ? "event_id" : eventId
						.toString();
			}
			// Added by muzees for PBG and Upjohn to fetch No-USField BU
			String nonUSfieldBU = getNonUSBUID();
			String AllBUID = getAllBUID();
			// end
			 ...
			 * "v.first_name as firstName, "; sql +=
			 * "v.last_name as lastName, "; sql +=
			 * "v.territory_id as territoryId, "; sql +=
			...
			// Start of muzees for PBG and UPJOHN 2019
			if (userGroup.equalsIgnoreCase("SCE_GuestTrainer_NonMGR")
					|| userGroup.equalsIgnoreCase("SCE_GuestTrainer_MGR")) {
				if (!businessUnit.equalsIgnoreCase(AllBUID)) {
					sql += "where v.SALES_GROUP in (SELECT DISTINCT SALES_ORGANIZATION FROM BU_SALESORG_MAPPING WHERE MAPPED_BU='"
							+ businessUnit + "') and ";
				} else {
					sql += "where ";
				}
			} else {// end of muzees PBG and UPJOHN
				sql += "where ";
			}
			sql += "v.emplId in ";
			sql += "( ";
			...
			if (SCEUtils.isFieldNotNullAndComplete(lastName)
				...
				/*
				 * start code change done for displaying CSO while searching
				 * through firstName and lastName BY Sanjeev
				 */
				if (userGroup.equalsIgnoreCase("SCE_Administrators")
						|| userGroup.equalsIgnoreCase("SCE_TrainingTeam")
						|| businessUnit.equalsIgnoreCase(nonUSfieldBU)
						|| businessUnit.equalsIgnoreCase(AllBUID)) {
					sql += " union";
					sql += " select ";
					...
					sql += " where ";
					sql += "(";
					sql += "upper(l.last_name) like '"
							+ lastName.replaceAll("'", "''").toUpperCase()
									.trim() + "%'";
					sql += " and ";
					sql += "upper(l.first_name) like '"
							+ firstName.replaceAll("'", "''").toUpperCase()
									.trim() + "%'";
					sql += ")";
				}
				/*
				 * end code change done for displaying CSO while searching
				 * through firstName and lastName BY Sanjeev
				 */
				...
				if (userGroup.equalsIgnoreCase("SCE_Administrators")
					...
					sql += " sales_call.p2l_learners l ";
					sql += " where ";
					sql += "upper(l.last_name) like upper('"
							+ lastName.replaceAll("'", "''").toUpperCase()
									.trim() + "%')";
				}
				/*
				...
				// return sceControl.getAttendeesByEmplId(emplId.trim(),
				// isPassport,
				// lOrder, fOrder);
				return sceControl.getAttendeesBySearchCriteria(sql);
				...
				/*
				 * Start code change done for displaying CSO while searching
				 * through salesPositionId BY Sanjeev
				 */
				if (userGroup.equalsIgnoreCase("SCE_Administrators")
						|| userGroup.equalsIgnoreCase("SCE_TrainingTeam")
						|| businessUnit.equalsIgnoreCase(nonUSfieldBU)
	...
	public User[] getUsersByStatus(String userStatus, String userGroup) {
		return sceControl.getUsersByStatus(userStatus, userGroup);
	}

	public User[] getAllUsers(String userGroup) {
		return sceControl.getAllUsers(userGroup);
	}
			...
			for (int i = 0; i < objDescriptorArr.length; i++) {
				for (int j = 0; j < objSCEDetailArr.length; j++) {
					if (objDescriptorArr[i].getQuestionId().equals(
							objSCEDetailArr[j].getQuestionId())) {
						objSCEDetailArr[j]
	...
	public LegalQuestion[] getLegalQuestion(Integer templateVersionId)
			throws SCEException {
	...
	public EvaluationFormScore[] getEvaluationFormScore(
		...
		try {
			evaluationFormScores = sceControl
					.getEvaluationFormScore(templateVersionId);
		} catch (SQLException sqle) {
			// log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
		} catch (Exception e) {
	...
	public SCE[] getSCESHistoryByEventEmplId(Integer eventId, String emplId,
			String productName) throws SCEException {
		SCE[] SCESHistory = null;
		try {
			SCESHistory = sceControl.getSCESHistoryByEventEmplId(eventId,
					emplId, productName);
		} catch (SQLException sqle) {
			// log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
	...
	public SCE getNewSCE(Integer eventId, String course, String productName)
			throws SCEException {
		...
		SCEDetail[] objSCEDetailArr = sceControl.getSCEDetailsLatest(eventId,
				course, productName);
		evaluationTemplateId = objSCEDetailArr[0].getTemplateVersionId();

		// Added try/catch clause by Shinoy, 28Nov2011
		try {
			objBusinessRulesArr = sceControl
					.getBusinessRules(evaluationTemplateId);
			objLegalQuestionArr = sceControl
					.getLegalQuestion(evaluationTemplateId);

			// EmailTemplate[] objEmailTemplateArr =
			// sceControl.getEmailTemplate(evaluationTemplateId);

			objEvaluationFormScoreArr = sceControl
					.getEvaluationFormScore(evaluationTemplateId);

			// Get The Descriptors For All The Questions and Assign Them
			Descriptor[] objDescriptorArr = sceControl.getDescriptorsLatest(
					eventId, course);
			...
			for (int i = 0; i < objSCEDetailArr.length; i++) {
				if (i == 0) {
					objSCE.setTemplateVersionId(objSCEDetailArr[i]
							.getTemplateVersionId());
					objSCE.setFormTitle(objSCEDetailArr[i].getFormTitle());
					objSCE.setEventId(eventId);
					objSCE.setPrecallComments(objSCEDetailArr[i]
							.getPrecallComments());
					objSCE.setPostcallComments(objSCEDetailArr[i]
							.getPostcallComments());
					objSCE.setHlcCritical(objSCEDetailArr[i].getHlcCritical());
					objSCE.setLegalFG(objSCEDetailArr[i].getLegalFG());
					objSCE.setConflictOverallScore(objSCEDetailArr[i]
							.getConflictOverallScore());
					objSCE.setHLCCriticalValue(objSCEDetailArr[i]
							.getHLCCriticalValue());
					...
					objSCE.setPostcallLabel(objSCEDetailArr[i]
							.getPostcallLabel());

					/* Added by Ankit 27April2016 */
					objSCE.setTemplatetitle(objSCEDetailArr[i]
							.getTemplatetitle());
					objSCE.setCallImageDisplay(objSCEDetailArr[i]
							.isCallImageDisplay());
					objSCE.setOverallEvaluationLable(objSCEDetailArr[i]
							.getOverallEvaluationLable());
					System.out.println("Added template title :"
							+ objSCEDetailArr[i].getTemplatetitle()
							+ " to objSCE object in getNewSCE()");

					objSCE.setPreCallImage(objSCEDetailArr[i].getPreCallImage());
					objSCE.setPostCallImage(objSCEDetailArr[i]
							.getPostCallImage());
					objSCE.setCallLabelDisplay(objSCEDetailArr[i]
							.getCallLabelDisplay());
					objSCE.setCallLabelValue(objSCEDetailArr[i]
							.getCallLabelValue());
	...
	public void giveAutoCredit(String emplId, Integer eventId,
		...
		if ((emplId != null && !emplId.trim().equals(""))
				&& (productCode != null && !productCode.trim().equals(""))) {
			// // System.out.println("Length of params array = "+params.length);
			sceControl.giveAutoCredit(emplId, eventId, productCode, product,
					course, classroom, tableName, submittedByEmplId);
		}
	}

	public SCEReport[] getPendingAttendeesByCourseAndClass(Integer eventId,
			Integer courseId, String classroom) {
		String sql = "select ";
		sql += "v.EMPLID as emplId, ";
		 ...
		 */
		SCEReport[] sceReportAll = sceControl
				.getPendingAttendeesByCourseAndClass(sql);
	...
	public Attendee[] getPendingAttendeesByFullcriteria(Integer eventId,
			Date trainingDate, String productCode, String course,
		...
		if (excludeEmplId != null) {
			...
			// attendees = sceControl.getOtherPendingAttendees(eventId,
			// trainingDate, productCode, course, excludeEmplId);
			attendees = sceControl.getOtherPendingAttendeesByFullCriteria(
					eventId, trainingDate, productCode, course, classroom,
					table, excludeEmplId);
			List attendeesList = Arrays.asList(attendees);
			Collections.sort(attendeesList);
			attendees_sort = (Attendee[]) attendeesList
					.toArray(new Attendee[attendees.length]);
		} else {
			attendees = sceControl.getPendingAttendeesByFullCriteria(eventId,
					trainingDate, productCode, course, classroom, table);
			List attendeesList = Arrays.asList(attendees);
			Collections.sort(attendeesList);
			attendees_sort = (Attendee[]) attendeesList
					.toArray(new Attendee[attendees.length]);
	...
	/*public String getempbyNTID(String ntid) {
		String emplid = null;
		// Get User From SCE Users Table
		emplid = sceControl.getempbyNTID(ntid);
		if (emplid== null) {
			// Get By EmplId
	...
	public EmailTemplate getEmailTemplate(Integer templateVersionId,
			String overallScore) throws SCEException {
		EmailTemplate emailTemplate = null;
		try {

			emailTemplate = sceControl.getEmailTemplate(templateVersionId,
					overallScore);
	...
	public void saveSCE(SCE objSCE, SCEDetail[] objSCEDetailArr,
			LegalQuestion[] legalDetailArr) throws SCEException {
		...
		try {
			...
			Integer sceId = null;
			if (objSCE != null) {
				if (objSCE.getId() == null
						|| objSCE.getId().intValue() == SCEConstants.SCE_INVALID_ID
								.intValue()) {
					isInsert = true;
				}
				...
				if (isInsert) {
					...
					if (legalDetailArr != null) {
						for (int i = 0; i < legalDetailArr.length; i++) {
							...
							String legalQuestionValue = legalDetail
									.getLegalQuestionValue();
							sceControl.insertLegalDetail(sceId, id,
									legalQuestionValue);
						}
					}
					...
					if (legalDetailArr != null) {
						for (int i = 0; i < legalDetailArr.length; i++) {
							...
							String legalQuestionValue = legalDetail
									.getLegalQuestionValue();
							sceControl.updateLegalDetail(sceId, id,
									legalQuestionValue);
						}
					}
				}
	...
	public boolean checkIfDuplicate(SCE objSCE, SCEDetail[] objSCEDetailArr)
			throws SCEException {
		if (objSCE != null) {
			// Get Last Submitted SCE
			SCE oSCE = getLatestSCE(objSCE.getEmplId(), objSCE.getEventId(),
					objSCE.getProductCode(), objSCE.getStatus());
			if (oSCE != null) {
				// Check SCE

				if (((oSCE.getTemplateVersionId() == null && objSCE
						.getTemplateVersionId() == null) || (oSCE
						.getTemplateVersionId() != null && oSCE
						.getTemplateVersionId().equals(
								objSCE.getTemplateVersionId())))
						&& ((oSCE.getPrecallRating() == null && objSCE
								.getPrecallRating() == null) || (oSCE
								.getPrecallRating() != null && oSCE
								.getPrecallRating().equals(
										objSCE.getPrecallRating())))
						&& ((oSCE.getPrecallComments() == null && objSCE
								.getPrecallComments() == null) || (oSCE
								.getPrecallComments() != null && oSCE
								.getPrecallComments().equals(
										objSCE.getPrecallComments())))
						&& ((oSCE.getPostcallRating() == null && objSCE
								.getPostcallRating() == null) || (oSCE
								.getPostcallRating() != null && oSCE
								.getPostcallRating().equals(
										objSCE.getPostcallRating())))
						&& ((oSCE.getPostcallComments() == null && objSCE
								.getPostcallComments() == null) || (oSCE
								.getPostcallComments() != null && oSCE
								.getPostcallComments().equals(
										objSCE.getPostcallComments())))
						&& ((oSCE.getComments() == null && objSCE.getComments() == null) || (oSCE
								.getComments() != null && oSCE.getComments()
								.equals(objSCE.getComments())))
						&& ((oSCE.getOverallRating() == null && objSCE
								.getOverallRating() == null) || (oSCE
								.getOverallRating() != null && oSCE
								.getOverallRating().equals(
										objSCE.getOverallRating())))
						&& ((oSCE.getOverallComments() == null && objSCE
								.getOverallComments() == null) || (oSCE
								.getOverallComments() != null && oSCE
								.getOverallComments().equals(
										objSCE.getOverallComments())))
				/*
				 * && ( (oSCE.getOverallScore() == null &&
				 * objSCE.getOverallScore() == null) || (oSCE.getOverallScore()
				 * != null &&
				 * oSCE.getOverallScore().equals(objSCE.getOverallScore())) )
				 */
				) {
				...
				SCEDetail[] oSCEDetailArr = sceControl
						.getSCEDetailsBySCEId(oSCE.getId());
				if (oSCEDetailArr != null && objSCEDetailArr != null) {
					...
					for (int i = 0; i < oSCEDetailArr.length; i++) {
						for (int j = 0; j < oSCEDetailArr.length; j++) {
							if (oSCEDetailArr[i].getQuestionId() != null
									&& oSCEDetailArr[i].getQuestionId().equals(
											objSCEDetailArr[j].getQuestionId())) {
								// Compare Now
								if (((oSCEDetailArr[i].getQuestionRating() == null && objSCEDetailArr[j]
										.getQuestionRating() == null) || (oSCEDetailArr[i]
										.getQuestionRating() != null && oSCEDetailArr[i]
										.getQuestionRating().equals(
												objSCEDetailArr[j]
														.getQuestionRating())))
										&& ((oSCEDetailArr[i]
												.getQuestionComments() == null && objSCEDetailArr[j]
												.getQuestionComments() == null) || (oSCEDetailArr[i]
												.getQuestionComments() != null && oSCEDetailArr[i]
												.getQuestionComments()
												.equals(objSCEDetailArr[j]
														.getQuestionComments())))
										&& ((oSCEDetailArr[i]
												.getQuestionScore1() == null && objSCEDetailArr[j]
												.getQuestionScore1() == null) || (oSCEDetailArr[i]
												.getQuestionScore1() != null && oSCEDetailArr[i]
												.getQuestionScore1()
												.equals(objSCEDetailArr[j]
														.getQuestionScore1())))
										&& ((oSCEDetailArr[i]
												.getQuestionScore2() == null && objSCEDetailArr[j]
												.getQuestionScore2() == null) || (oSCEDetailArr[i]
												.getQuestionScore2() != null && oSCEDetailArr[i]
												.getQuestionScore2()
												.equals(objSCEDetailArr[j]
														.getQuestionScore2())))
										&& ((oSCEDetailArr[i]
												.getQuestionScore3() == null && objSCEDetailArr[j]
												.getQuestionScore3() == null) || (oSCEDetailArr[i]
												.getQuestionScore3() != null && oSCEDetailArr[i]
												.getQuestionScore3()
												.equals(objSCEDetailArr[j]
														.getQuestionScore3())))) {
	...
	public SCE getLatestSCE(String emplId, Integer eventId, String productCode,
			String status) throws SCEException {
		try {
			Integer evaluationTemplateId;

			SCE objSCE = sceControl.getLatestSCE(emplId, eventId, productCode,
					status);

			if (objSCE != null) {
				evaluationTemplateId = objSCE.getTemplateVersionId();

				BusinessRule[] objBusinessRulesArr = sceControl
						.getBusinessRules(evaluationTemplateId);

				LegalQuestion[] objLegalQuestionArr = sceControl
						.getLegalQuestion(evaluationTemplateId);

				// EmailTemplate[] objEmailTemplateArr =
				// sceControl.getEmailTemplate(evaluationTemplateId);

				EvaluationFormScore[] objEvaluationFormScoreArr = sceControl
						.getEvaluationFormScore(evaluationTemplateId);

				SCEDetail[] objSCEDetailArr = sceControl
				...
				if (objSCEDetailArr != null) {
					...
					double tmp = 0.0;
					for (int i = 0; i < objSCEDetailArr.length; i++) {
						if (SCEConstants.QT_SCORE
								.equalsIgnoreCase(objSCEDetailArr[i].getType())
								|| (SCEConstants.QT_SCORE_FETCH
										.equalsIgnoreCase(objSCEDetailArr[i]
												.getType()) && SCEConstants.ST_SUBMITTED
										.equalsIgnoreCase(objSCE.getStatus()))) {
							objSCEDetailArr[i]
									.setAverageScore(objSCEDetailArr[i]
											.computeAverageScore());
							if (objSCEDetailArr[i].getWeightedScore() != null) {
								tmp += objSCEDetailArr[i].getWeightedScore()
										.doubleValue();
								hasTotal = true;
							}
						}
					...
					if (SCEConstants.ST_SUBMITTED.equalsIgnoreCase(objSCE
							.getStatus())) {
				...
				Descriptor[] objDescriptorArr = sceControl
						.getDescriptorsBySCEId(objSCE.getId());
				if (objDescriptorArr != null && objSCEDetailArr != null) {
					for (int i = 0; i < objDescriptorArr.length; i++) {
						for (int j = 0; j < objSCEDetailArr.length; j++) {
							if (objDescriptorArr[i].getQuestionId().equals(
				...
				if (objSCEDetailArr != null) {
					for (int i = 0; i < objSCEDetailArr.length; i++) {
						if (i == 0) {
							objSCE.setTemplateVersionId(objSCEDetailArr[i]
									.getTemplateVersionId());
							objSCE.setFormTitle(objSCEDetailArr[i]
									.getFormTitle());
							objSCE.setEventId(eventId);
							objSCE.setPrecallComments(objSCEDetailArr[i]
									.getPrecallComments());
							objSCE.setPostcallComments(objSCEDetailArr[i]
									.getPostcallComments());
							objSCE.setHlcCritical(objSCEDetailArr[i]
									.getHlcCritical());
							objSCE.setLegalFG(objSCEDetailArr[i].getLegalFG());
							objSCE.setConflictOverallScore(objSCEDetailArr[i]
									.getConflictOverallScore());
	...
	public EvaluationFormScore getEvaluationFormValues(
			Integer templateVersionId, String overallScore) throws SCEException {
		EvaluationFormScore evaluationFormScore = null;

		try {
			evaluationFormScore = sceControl.getEvaluationFormValues(
					templateVersionId, overallScore);
	...
	public HashMap getAllScores(Integer templateVersionId) throws SCEException {
		...
		try {
			...
			ScoringSystem[] scoringSytem = sceControl
					.getAllScores(templateVersionId);
			scoreList = new LinkedHashMap();
			if (scoringSytem != null && scoringSytem.length > 0) {
				for (int i = 0; i < scoringSytem.length; i++) {
					scoreList.put("", "");
					scoreList.put(scoringSytem[i].getScoreLegend(),
							scoringSytem[i].getScoreLegend() + " "
									+ scoringSytem[i].getScoreValue());
				}
			}
		} catch (SQLException sqle) {
			// log.error(LoggerHelper.getStackTrace(sqle));
	...
	public void uploadForm(com.pfizer.sce.beans.EvaluationForm evalForm,
			java.lang.Integer sceId) throws SCEException {
	...
	public Learner[] getLearnersByCourseAndDate(String course, String frmDate,
			String toDate) throws SCEException {
		try {
			return sceControl.getLearnersByCourseAndDate(course, frmDate,
					toDate);
	...
	public com.pfizer.sce.beans.Learner getLearnerByEmailCourse(String email,
			String course) throws SCEException {
		try {
	...
	public void updateUserAcceptInvite(String eventName, String emailSel)
			throws SCEException {
		try {
			GuestTrainer trainers = sceControl.getGTforAccess(emailSel);
			String ntID = trainers.getNtid();
			/*
			 * SQLParameter[] params = new SQLParameter[4]; params[0] = new
			 * SQLParameter(ntID, Types.VARCHAR, SQLParameter.IN); params[1] =
			 * new SQLParameter(eventName, Types.VARCHAR, SQLParameter.IN);
			 * params[2] = new SQLParameter(emailSel, Types.VARCHAR,
			 * SQLParameter.IN); params[3] = new SQLParameter("A",
			 * Types.VARCHAR, SQLParameter.IN);
			 */
	...
	public void updateLastEventDate(String product, String trainerEmail,
			String updateDate) throws SCEException {
		try {
	...
	public String getDateToUpdate(String eventName, String product,
			String trainerEmail) throws SCEException {
	...
	// code by manish to add approvers
	public User[] getApprovers() throws SCEException {
		System.out.println("Entering SCEManager Implementation");
		User[] lmsMapping = null;
		try {
			/*
			 * System.out .println(
			 * "Entering SCEManager Implementation and before executing method"
			 * );
			 */
			lmsMapping = sceControl.getApprovers();
			/*
			 * System.out .println(
			 * "Entering SCEManager Implementation and after executing method");
	...
	public int saveRoles(String roleCd, String roleDescription)
		...
		int status = 0;
		try {
			status = sceControl.saveRoles(roleCd, roleDescription);
		} catch (Exception e) {
			throw new SCEException("error.sce.unknown", e);
		}
		return status;
	...
	public void deleteRole(String roleCd) throws SCEException {
		// int status = 0;

		try {
			// status =
			sceControl.deleteRole(roleCd);
			// return status;
		} catch (Exception e) {
			throw new SCEException("error.sce.unknown", e);
		}
	...
	/*
	 * code added by KUMAM1234 for Role Mapping to get all roles currently
	 * mapped in SCE for version 3.4 RFC#1136920
	 */
	// added by manish to hide form
	public int hideForm(String formId) throws SCEException {
		int status = 0;

		try {
			status = sceControl.hideForm(formId);
			return status;
		} catch (Exception e) {
			throw new SCEException("error.sce.unknown", e);
		}
	...
	// added by manish to unhide form
	public int unHideForm(String formId) throws SCEException {
		int status = 0;

		try {
			status = sceControl.unHideForm(formId);
			return status;
		} catch (Exception e) {
			throw new SCEException("error.sce.unknown", e);
		}
	...
	public String[] getAllRoles() throws SCEException {
		return sceControl.getAllRoles();
	}

	/* Changes done begin Release2 */
	/*
	 * public void gotoConfirmInvitation(String email,String choice,String
	 * selEvent) throws SCEException { String selEmail = email; String ifChoice
	 * = choice; String event = selEvent; String sql =""; sql +=
	 * "update guest_trainer_event SET REP_ISACCEPTED = '"+ ifChoice
	 * +"' where REP_EMAIL = '"+ email + "'  and EVENT_NAME ='" + event + "'";;
	 * 
	 * 
	 * //if GT has rejected the invite its temporary acces is to be invoked
	 * if(ifChoice.equalsIgnoreCase("N")) {
	 * 
	 * try { String sqlQ = ""; GuestTrainer trainers =
	 * sceControl.getGTforAccess(selEmail); String ntID = trainers.getNtid();
	 * 
	 * SQLParameter[] params = new SQLParameter[4]; params[0] = new
	 * SQLParameter(ntID, Types.VARCHAR, SQLParameter.IN); params[1] = new
	 * SQLParameter(event, Types.VARCHAR, SQLParameter.IN); params[2] = new
	 * SQLParameter(selEmail, Types.VARCHAR, SQLParameter.IN); params[3] = new
	 * SQLParameter("R", Types.VARCHAR, SQLParameter.IN); //This procedure is
	 * called for revoking temporary access to GT's who are not in SCE system to
	 * enable them to accept or reject invite
	 * sceControl.gotoCallProcToRevoke(ntID,event,selEmail,"R");
	 * 
	 * } catch (SQLException sqle) {
	 * log.error(LoggerHelper.getStackTrace(sqle)); throw new
	 * SCEException("error.sce.database.communication",sqle); }catch (Exception
	 * e) { log.error(LoggerHelper.getStackTrace(e)); throw new
	 * SCEException("error.sce.unknown",e); } }
	 * 
	 * sceControl.gotoConfirmEmailSent(sql); }
	 */

	public void gotoConfirmInvitation(String email, String choice,
			String selEvent, String productSel) throws SCEException {
		...
		product = productSel;
		String sql = "";
		sql += "update guest_trainer_event SET REP_ISACCEPTED = '" + ifChoice
				+ "' where REP_EMAIL = '" + email + "'  and EVENT_NAME ='"
				+ event + "' and PRODUCT='" + product + "'";

		/*
		 * sql += " update guest_trainer_event SET REP_ISACCEPTED = '"+ ifChoice
		 * +"',PRODUCT='"+product+"' " +" where REP_EMAIL = '"+ email + "'  "
		 * +" and EVENT_NAME ='" + event + "' "
		 * +" and ( PRODUCT is null  or PRODUCT = '') "
		 * +" and ( REP_ISACCEPTED is null  or REP_ISACCEPTED = '')"
		 * +" and MAP_ID = (" +
		 * "                select max(MAP_ID) from guest_trainer_event where REP_EMAIL = '"
		 * + email + "'" +"                and EVENT_NAME ='" + event + "' "
		 * +"                and ( PRODUCT is null  or PRODUCT = '') "
		 * +"                and ( REP_ISACCEPTED is null or REP_ISACCEPTED = '')"
		 * +"              )" ;
		 */
		...
		if (ifChoice.equalsIgnoreCase("N")) {
			...
			try {
				 ...
				 * SQLParameter[] params = new SQLParameter[4]; params[0] = new
				 * SQLParameter(ntID, Types.VARCHAR, SQLParameter.IN); params[1]
				 * = new SQLParameter(event, Types.VARCHAR, SQLParameter.IN);
				 * params[2] = new SQLParameter(selEmail, Types.VARCHAR,
				 * SQLParameter.IN); params[3] = new SQLParameter("R",
				 * Types.VARCHAR, SQLParameter.IN);
				 */
				// This procedure is called for revoking temporary access to
				// GT's who are not in SCE system to enable them to accept or
				// reject invite
				sceControl.gotoCallProcToRevoke(ntID, event, selEmail, "R");
	...
	public void acceptGTinviteManual(String email, String choice,
			String selEvent, String productSel) throws SCEException {
		String selEmail = email;
		String ifChoice = choice;
		String event = selEvent;
		...
		sql += "update guest_trainer_event SET REP_ISACCEPTED = '" + ifChoice
				+ "' where REP_EMAIL IN (" + email + ") and EVENT_NAME ='"
				+ event + "' and PRODUCT='" + product + "'";

		/*
		 * sql += " update guest_trainer_event SET REP_ISACCEPTED = '"+ ifChoice
		 * +"',PRODUCT='"+product+"' " +" where REP_EMAIL IN (" + email + ") "
		 * +" and EVENT_NAME ='" + event + "' "
		 * +" and ( PRODUCT is null  or PRODUCT = '') "
		 * +" and ( REP_ISACCEPTED is null  or REP_ISACCEPTED != '"+ ifChoice
		 * +"') " +" and MAP_ID IN (" +
		 * "                select max(MAP_ID) from guest_trainer_event where REP_EMAIL IN ("
		 * + email + ") " +"                and EVENT_NAME ='" + event + "' "
		 * +"                and ( PRODUCT is null  or PRODUCT = '') "
		 * +"                and ( REP_ISACCEPTED is null or REP_ISACCEPTED != '"
		 * + ifChoice +"') group by REP_EMAIL   " +"              )" ;
		 */
	...
	/*
	 * Added By Ankit on 4 September 2015 for fetching Map_id from
	 * GUEST_TRAINER_EVENT
	 */
	...
	public void saveDateTimeSlots(String[][] dateTimeArray, String MapId,
			...
			throws Exception {
		try {
			sceControl.saveDateTimeSlots(dateTimeArray, MapId, hourList, email,
					event, product);
		} catch (Exception e) {
	...
	public void gotoConfirmEmailSent(String email, String event,
			String productName) throws SCEException {
		...
		for (int i = 0; i < x.length; i++) {
			...
			String emailSel = x[i];
			String sql = "";
			// This is for mapping GT to a particular event and store its
			// statuses corresponding to it
			/* sanjeev send product */
			sql += "INSERT INTO guest_trainer_event (REP_EMAIL_ISSENT,REP_EMAIL,PRODUCT,EVENT_NAME,MAP_ID) VALUES ('Y','"
					+ emailSel
					+ "', '"
					+ productName
					+ "','"
					+ eventName
					+ "' ,trainer_event_mapping_id.nextval)";
			sceControl.gotoConfirmEmailSent(sql);
			...
			try {
				...
				/*
				 * SQLParameter[] params = new SQLParameter[5]; params[0] = new
				 * SQLParameter(fName, Types.VARCHAR, SQLParameter.IN);
				 * params[1] = new SQLParameter(lName, Types.VARCHAR,
				 * SQLParameter.IN); params[2] = new SQLParameter(emailSel,
				 * Types.VARCHAR, SQLParameter.IN); params[3] = new
				 * SQLParameter(ntID, Types.VARCHAR, SQLParameter.IN); params[4]
				 * = new SQLParameter(startDate, Types.DATE, SQLParameter.IN);
				 */
				// This procedure is called for giving temporary access to GT's
				// who are not in SCE system to enable them to accept or reject
				// invite
				sceControl.gotoCallProcToAccess(fName, lName, emailSel, ntID,
						startDate);
				;
	...
	public void updateTrainerLearnerMappingEmailSent(String eventName,
			String courseStartDate, String learnerEmailList, String productName)
			throws SQLException, SCEException {
		String[] learnerEmail = learnerEmailList.split(";");
		try {
			for (int i = 0; i < (learnerEmail.length - 1); i++) {
				sceControl.updateTrainerLearnerMappingEmailSent(eventName,
						courseStartDate, learnerEmail[i], productName);
			}
		} catch (Exception e) {
			// log.error(LoggerHelper.getStackTrace(e));
			throw new SCEException("error.sce.unknown", e);
	 ...
	 * try { return sceControl.getGTByEvent(event);
	 * 
	 * } catch (SQLException sqle) { //
	 * log.error(LoggerHelper.getStackTrace(sqle)); throw new
	 * SCEException("error.sce.database.communication", sqle); } catch
	 * (Exception e) { // log.error(LoggerHelper.getStackTrace(e)); throw new
	 * SCEException("error.sce.unknown", e); } }
	 */
	...
	public LegalConsentTemplate getLegalTemplateById(int lcId)
		...
		try {
			...
			// System.out.println("publish flag"+
			// legalConsentTemplate.getPublishFlag());
	...
	public TemplateVersion getTemplateVersionByIdNew(Integer templateVersionId)
		...
		try {
			...
			templateVersion = sceControl
					.getTemplateVersionByIdNew(templateVersionId);
	...
	public EvaluationFormScore[] getEvaluationFormScoresByTemplateVersionId(
		...
		try {
			...
			for (int count = 0; count < evaluationFormScores.length; count++) {
				...
				// System.out.println("the scoring system identifier is"+
				...
				// System.out.println("the email publish flag is"+
				// emailPublishFlag);
	...
	public void saveTemplateVersionNew(TemplateVersion templateVersion,
			String pageName, boolean isModified, boolean isAlreadyMapped)
			throws SCEException {
		...
		try {
			if (pageName.equals("edit")) {
				 ...
				 * System
				 * .out.println("templateVersion.getPublishFlag:"+templateVersion
				 * .getPublishFlag());
				 * 
				 * System.out.println("isAlreadyMapped:"+isAlreadyMapped);
				 * System.out.println("isModified:"+isModified);
				 * System.out.println
				 * ("getPublishflag:"+templateVersion.getPublishFlag());
				 */

				// isAlreadyMapped = true;

				if (isAlreadyMapped == true && isModified == true
					...
					Integer currentTemplateVersion = sceControl
							.getCurrentVersion(templateVersion.getTemplateId());
			...
			while (litr.hasNext()) {
				...
				System.out.println("After score comment...Flag value is: "
						+ scorecommentFlag);
				...
				System.out.println("After save EvaluationFormScore");
	...
	public EmailTemplate checkEmailTemplate(String scoringSystemIdentifier,
			String templateName) throws SCEException {
		EmailTemplate emailTemplate = null;
		try {
			emailTemplate = sceControl.checkEmailTemplate(
	...
	public void insertIntoEmailTemplate(TemplateVersion templateVersion,
			...
			String score_value) throws SCEException {

		try {
			sceControl.insertIntoEmailTemplate(templateVersion,
	...
	public void updateEmailTemplate(String templateName,
		...
		try {
			sceControl.updateEmailTemplateVersion(templateName,
	...
	public void insertSCEComment(String sceId, String activity_id,
			...
			SQLException {
		try {
			sceControl.insertSCEComment(sceId, activity_id, phaseNo, rep_id,
					comment1, comment2, comment3, enteredby1, enteredby2,
					enteredby3, date1, date2, date3, submitted_by,
					submitted_date);
		} catch (Exception e) {
			// log.error(LoggerHelper.getStackTrace(e));
			throw new SCEException("error.sce.unknown", e);
	...
	public void updateSCEComment(String sceId, String activity_id,
			...
			SQLException {
		try {
			sceControl.updateSCEComment(sceId, activity_id, phaseNo, rep_id,
					comment1, comment2, comment3, enteredby1, enteredby2,
					enteredby3, date1, date2, date3, submitted_by,
					submitted_date);
		} catch (Exception e) {
	...
	public SCEComment getExistingSCEComment(String sceid, String activityid,
			String repid) throws SCEException, SQLException {
		try {
	...
	public Integer isSCEComment(String phaseid, String sceid, String repid)
			throws SCEException, SQLException {
		try {
	...
	public String getSceFeedbackId(String trackid) throws SCEException,
		...
		try {
			...
			System.out.println("getSceFeedbackId------2--"
					+ sceControl.getSceFeedbackId(trackid));
	...
	public CourseEvalTemplateMapping[] deleteSelLmsRecord(Integer delLmsId,
			Integer selTemplateId) throws SCEException, SQLException {
	...
	public CourseEvalTemplateMapping[] getAllMappingsForTemplate(
			Integer evalTemplateId) throws SCEException, SQLException {
		CourseEvalTemplateMapping[] mappingsForTemplates = null;
		try {

			mappingsForTemplates = sceControl
					.getAllMappingsForTemplate(evalTemplateId);
	...
	public CourseEvalTemplateMapping[] saveMappedRecords(String emplId,
		...
		try {
			...
			lmsMappings = sceControl.saveMappedRecords(emplId,
					excludedActivityPk, selectedTemplateId,
					selectedTemplateName, selectedCourseIds);
	...
	public CourseDetails[] getCourseDetailsForActPk(String[] alreadyMappedRecs)
			throws SCEException {
		...
		try {
			// String array alreadyMappedRecs contains String values of Activity
			// PK
			if (alreadyMappedRecs != null) {
				...
				for (int i = 0; i < alreadyMappedRecs.length; i++) {
					...
					CourseDetails obj = sceControl
							.getCourseDetailsForActPk(excludeActPk);
	...
	public CourseEvalTemplateMapping[] getAlreadyMappedCourses(
		...
		try {
			alreadyMappedCourses = sceControl.getAlreadyMappedCourses(
					selCourseIds, selCourseCodes, selEvalTempId);
		} catch (Exception e) {
			// log.error(LoggerHelper.getStackTrace(e));
			throw new SCEException("error.sce.unknown", e);
		}
	...
	public WebExDetails[] getWebExDetailsByEventAndUsedBy(String eventName,
			String usedby) throws SCEException {
		WebExDetails[] webExDetails = null;
		try {
			webExDetails = sceControl.getWebExDetailsByEventAndUsedBy(
					eventName, usedby);
		} catch (SQLException sqle) {
			// log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
	...
	public TrainerLearnerMapping[] getCountOfEvalsPerTrainer(String eventName,
		...
		TrainerLearnerMapping[] lmsMapping = null;
		try {
			lmsMapping = sceControl.getCountOfEvalsPerTrainer(eventName,
					courseStartDate);
		} catch (SQLException sqle) {
			// log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
	...
	public TrainerLearnerMapping[] getNoOfMailSentByEventTrainer(
		...
		TrainerLearnerMapping[] lmsMapping = null;
		try {
			lmsMapping = sceControl.getNoOfMailSentByEventTrainer(eventName,
					trainerEmail, courseStartDate);
		} catch (SQLException sqle) {
			// log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
		} catch (Exception e) {
	...
	public void gotosaveTrainerLearnerMap(String selCourseName,
			String selTrainer, String selLearner, String trainerEmail,
			String learnerEmail, String whichEventName, String learnerLOC,
			String trainerLOC, String dates) throws SQLException, SCEException {
		try {
			sceControl.gotoSaveEventMapping(selCourseName, selTrainer,
					selLearner, trainerEmail, learnerEmail, whichEventName,
					learnerLOC, trainerLOC, dates);
		} catch (Exception e) {
			// log.error(LoggerHelper.getStackTrace(e));
			throw new SCEException("error.sce.unknown", e);
		}
	...
	public TrainerLearnerMapping[] getAlreadyMappedTrainer(String eventName,
		...
		TrainerLearnerMapping[] lmsMapping = null;
		try {
			lmsMapping = sceControl.getAlreadyMappedTrainer(eventName,
					courseStartDate);
		}
	...
	public GuestTrainer[] getTrainerForAuto(String produtSel, String eventName,
		...
		GuestTrainer[] guestTrainers = null;
		try {
			guestTrainers = sceControl.getTrainerForAuto(produtSel, eventName,
					courseStartDate);
		}
	...
	public TrainerLearnerMapping[] gotoCheck(String learnerEmail,
		...
		try {
			...
			lmsMapping = sceControl.gotoCheck(learnerEmail, whichEventName,
					selCourseName, selCourseDateName);
	...
	public TrainerLearnerMapping[] getNoOfMailSentByEventTrainer(
			String eventName, String trainerEmail) throws SCEException {
	...
	public GuestTrainer getGTFromAtlasByEmailProduct(String email,
			String product) throws SCEException {
		try {
	...
	public SCE getNewSCEPhase(Integer eventId, String course)
			throws SCEException {

		SCE objSCE = new SCE();

		try {
			SCEDetail[] objSCEDetailArr = sceControl.getSCEDetailsLatestPhase(
					eventId, course);
	...
	public SCE[] getSCEHistoryForTRT(Integer eventId, String emplId,
			String productCode) throws SCEException {
		SCE[] SCESHistoryForTRT = null;
		try {
			SCESHistoryForTRT = sceControl.getSCEHistoryForTRT(eventId, emplId,
					productCode);
		} catch (SQLException sqle) {
			log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
	...
	public void saveSCEPhase(SCE objSCE, SCEDetail[] objSCEDetailArr)
			throws SCEException {
		boolean isInsert = false;
		if (objSCE != null) {
			if (objSCE.getId() == null
	...
	/*
	 * //Introduced by Arpit as part of SCE Migration on 5th August 2014 public
	 * HashMap getAllEvaluationTemplatesMap() throws SCEException { //
	 * System.out.println("inside getAllEvaluationTemplatesMap method ");
	 * EmailTemplate[] emailTemplates = null; HashMap templateMap = new
	 * LinkedHashMap(); try { //
	 * System.out.println("calling getevaluationTemplates method ");
	 * emailTemplates = sceControl.getevaluationTemplates(); templateMap.put(new
	 * Integer(SCEConstants.ALL_EVENT_ID), "--Select--"); if (emailTemplates !=
	 * null && emailTemplates.length > 0) { for (int i = 0; i <
	 * emailTemplates.length; i++) templateMap.put(
	 * emailTemplates[i].getEvaluationTemplateId(),
	 * emailTemplates[i].getEvaluationTemplateName()); } } catch (Exception e) {
	 * // log.error(LoggerHelper.getStackTrace(e)); throw new
	 * SCEException("error.sce.unknown", e); } //
	 * System.out.println("The map returned is" + templateMap); return
	 * templateMap; }
	 * 
	 * // Introduced by Arpit as part of SCE Migration on 5th August 2014 public
	 * HashMap getScoringOptions(Integer evaluationTemplateId) throws
	 * SCEException { // HashMap scoringOptionsMap = new LinkedHashMap(); //
	 * System.out.println("inside getScoringOptions method"); List
	 * scoringOptionsList = new ArrayList(); EmailTemplate[] emailTemplates =
	 * null; String[] scoringOptions = null; String[] scoreValue = null; HashMap
	 * scoringOptionsMap = new HashMap(); try { //
	 ...
	 * // System.out.println("value of scoring options is"+scoringOptions ); for
	 * (int i = 0; i < scoringOptions.length; i++) { scoreValue =
	 * sceControl.getScoreValues(scoringOptions[i]); //
	 * System.out.println("scoreValue is "+ scoreValue);
	 * scoringOptionsMap.put(scoringOptions
	 * [i],getScoringOptionString(scoreValue)); } //
	 * System.out.println("completed the for loop"); } catch (SQLException sqle)
	 * { // log.error(LoggerHelper.getStackTrace(sqle)); throw new
	 * SCEException("error.sce.database.communication", sqle); } catch
	 * (Exception e) { // log.error(LoggerHelper.getStackTrace(e)); throw new
	 * SCEException("error.sce.unknown", e); } //
	 * System.out.println("before return"); return scoringOptionsMap; }
	 * 
	 * 
	 * // Introduced by Arpit as part of SCE Migration on 5th August 2014 public
	 * String getScoringOptionString(String[] scoreValue) { //
	 * System.out.println("within getScoringOPtionString method"); StringBuffer
	 * sb = new StringBuffer();
	 * 
	 * for (int i = 0; i < (scoreValue.length); i++) { //
	 * System.out.println(scoreValue[i]); sb.append(scoreValue[i]);
	 * sb.append(","); } String score = sb.toString(); //
	 * System.out.println("value of first score is"+ score); int lastIndx =
	 * score.length() - 1; score = score.substring(0, lastIndx); //
	 * System.out.println("value of final score is "+ score); return score; }
	 * 
	 * // Introduced by Arpit as part of SCE Migration on 7th August 2014 public
	 * EmailTemplate[] searchEmailTemplates(Integer evaluationTemplateId,String
	 * scoringOption) throws SCEException { //
	 * System.out.println("value of evaluationTemplateId "+
	 * evaluationTemplateId); // System.out.println("value of scoringOption"+
	 * scoringOption); EmailTemplate[] emailtemplates = null; try{
	 * emailtemplates =
	 * sceControl.getAllEmailTemplates(evaluationTemplateId,scoringOption);
	 * }catch (SQLException sqle) {
	 * //log.error(LoggerHelper.getStackTrace(sqle)); throw new
	 * SCEException("error.sce.database.communication",sqle); }catch (Exception
	 * e) { //log.error(LoggerHelper.getStackTrace(e)); throw new
	 * SCEException("error.sce.unknown",e); } return emailtemplates; }
	 * 
	 * 
	 * public String publishEmailTemplate(Integer emailTemplateId) throws
	 * SCEException {
	 * 
	 * String message = "The template has been published"; try{ SQLParameter[]
	 * params = new SQLParameter[2]; params[0] = new
	 * SQLParameter(emailTemplateId,Types.INTEGER,SQLParameter.IN); // params[1]
	 * = new SQLParameter(evaluationTemplateName,Types.VARCHAR,SQLParameter.IN);
	 * // params[2] = new
	 * SQLParameter(overallScore,Types.VARCHAR,SQLParameter.IN); //params[0] =
	 * new SQLParameter(new Integer(1),Types.INTEGER,SQLParameter.OUT);
	 * 
	 * Integer errorCodeSize= sceControl.publishEmailTemplate(emailTemplateId);
	 * 
	 * Integer errorCode = errorCodeSize;
	 * 
	 * if (errorCode != null) { if (errorCode.intValue() == 1) message =
	 * "Unable to publish email template"; else if(errorCode.intValue() == 0)
	 ...
	 * SCEException("error.sce.database.communication",sqle); }catch (Exception
	 * e) { //log.error(LoggerHelper.getStackTrace(e)); throw new
	 * SCEException("error.sce.unknown",e); } return message; }
	 * 
	 * 
	 * public EmailTemplateForm getSelectedEmailTemplate(Integer
	 * emailTemplateId) throws SCEException { EmailTemplateForm emailTemplate =
	 * null; try{ emailTemplate =
	 * sceControl.getSelectedEmailTemplate(emailTemplateId); }catch
	 * (SQLException sqle) { //log.error(LoggerHelper.getStackTrace(sqle));
	 * throw new SCEException("error.sce.database.communication",sqle); }catch
	 * (Exception e) { // log.error(LoggerHelper.getStackTrace(e)); throw new
	 * SCEException("error.sce.unknown",e); } return emailTemplate; }
	 */
	...
	public HashMap getScoringOptions(Integer evaluationTemplateId)
		...
		try {
			// System.out.println("Before calling retrieveScoringOptions method");
			scoringOptions = sceControl
					.retrieveScoringOptions(evaluationTemplateId);
			...
			for (int i = 0; i < scoringOptions.length; i++) {
				...
				// System.out.println("scoreValue is "+ scoreValue);
				scoringOptionsMap.put(scoringOptions[i],
						getScoringOptionString(scoreValue));
			}
	...
	public EmailTemplate[] searchEmailTemplates(Integer evaluationTemplateId,
			String scoringOption) throws SCEException {
		// System.out.println("value of evaluationTemplateId "+
		// evaluationTemplateId);
		// System.out.println("value of scoringOption"+ scoringOption);
		EmailTemplate[] emailtemplates = null;
		try {
			emailtemplates = sceControl.getAllEmailTemplates(
					evaluationTemplateId, scoringOption);
		} catch (SQLException sqle) {
			// log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
		} catch (Exception e) {
	...
	public String publishEmailTemplate(Integer emailTemplateId)
		...
		try {
			...
			errorCodeSize = sceControl.publishEmailTemplate(emailTemplateId,
					errorCodeSize);
	...
	public EmailTemplateForm getSelectedEmailTemplate(Integer emailTemplateId)
		...
		try {
			emailTemplate = sceControl
					.getSelectedEmailTemplate(emailTemplateId);
		} catch (SQLException sqle) {
			// log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
		} catch (Exception e) {
	...
	public String saveEmailTemplate(EmailTemplate emailTemplate)
		...
		String evaluationTemplateName = emailTemplate
				.getEvaluationTemplateName();
		...
		try {
			// emailtemplatesforVersion =
			// sceControl.getSelectedEmailTemplate(emailTemplateIdforVersion);
			emailtemplatesforVersion = sceControl
					.getlatestEmailTemplateVersion(evaluationTemplateName,
							scoringOption, overallScore);
			if (emailtemplatesforVersion == null) {

				// // System.out.println("email cc"
				// +emailtemplatesforVersion.getEmailSubject());
				// if (emailTemplateVersion.equalsIgnoreCase("Published"))

				Integer emailTemplateId = sceControl
				...
				emailTemplate.setEmailTemplateId(emailtemplatesforVersion
						.getEmailTemplateId());
	...
	public TrainerLearnerMapping[] getevaluationHours(String courseStartDate)
			throws SCEException {
		System.out.println("Entering SCEManager Implementation");
		TrainerLearnerMapping[] lmsMapping = null;
		try {
			System.out
					.println("Entering SCEManager Implementation and before executing method");
			lmsMapping = sceControl.getevaluationHours(courseStartDate);
			System.out
					.println("Entering SCEManager Implementation and after executing method");
	...
	public String getTimeSlots(String courseStartDate, String email)
			throws SCEException {
		System.out.println("Entering SCEManager Implementation");
		String lmsMapping = "";
		try {
	...
	// method added by manish to insert data into request access

	public void insertRequest(User user) {

		sceControl.insertRequest(user);
	}

	// end method to insert data into request access

	// method to update request

	public void updateRequest(User user) {

		sceControl.updateRequest(user);
	}

	// end method

	// code added by manish to update req user ststus

	public void updateRequestUserStatus(User user) {

		sceControl.updateRequestUserStatus(user);
	}

	// code end

	// code added by manish to reject req user ststus

	public void rejectRequestUserStatus(User user) {

		sceControl.rejectRequestUserStatus(user);
	}

	// code end

	// code added by manish to get request user

	public String getRequestUser(String ntId) throws SCEException {
		String user = "";
		System.out.println("NTID we got in Manager:" + ntId);
		try {

			user = sceControl.getRequestUser(ntId);
		} catch (Exception e) {
	...
	// code added by manish to get status of request user

	public String requestUserStatus(String ntId) throws SCEException {

		String status = "";
		System.out.println("NTID we got in Manager:" + ntId);
		try {

			status = sceControl.requestUserStatus(ntId);
		} catch (Exception e) {
	...
	// code by manish to remove approvers
	public void removeApprovers() {
		sceControl.removeApprover();
	}

	// code end

	// method added by manish to insert data into request_approvers

	public void insertApprover(User user) {

		sceControl.insertApprover(user);
	}

	// end method to insert data into request_approvers
	...
	public String checkIfReportingManager(String GuestTrainerEmplid,
		...
		String reportingCheck = null;

		try {
			reportingCheck = sceControl.checkIfReportingManager(
					GuestTrainerEmplid, CheckEmplId);
	...
	public String[] getProductForEventbyBU(String businessUnit)
		...
		try {
			...
			return sceControl.getProductForEventbyBu(businessUnit);
		}
	...
	public BU[] getAllMappedSalesOrg() throws SCEException {
		String nonUSfieldBU = getNonUSBUID();
		return sceControl.getAllMappedSalesOrg(nonUSfieldBU);
	}

	public String[] getSalesOrg() throws SCEException {
		return sceControl.getSalesOrg();
	}
	...
	public int saveBUMapping(String businessUnit, String salesOrganisation)
		...
		int status = 0;
		try {
			status = sceControl.saveBUMapping(businessUnit, salesOrganisation);
		} catch (Exception e) {
			throw new SCEException("error.sce.unknown", e);
		}
		return status;
	...
	public void deleteSalesOrg(String salesOrganiation) throws SCEException {
		// int status = 0;
		try {
			sceControl.deleteSalesOrg(salesOrganiation);
	...
	public void updateOverNightGT(User user) {
		sceControl.updateOverNightGT(user);
	}// end of MUZEES

	/* 2020 Q3: Start of MUZEES for multiple evalution */
	public boolean checkduplicate(SCE checkSCE, SCE newSCE) throws SCEException {
		try {
			return sceControl.checkduplicate(checkSCE, newSCE);
		} catch (Exception e) {
			throw new SCEException("error.sce.unknown", e);
		}
	}
	...
	public Date minRegistrationDate(String emplid, String eventid,
			String productCode) throws SCEException {
		try {
			return sceControl.minRegistrationDate(emplid, eventid, productCode);
		} catch (Exception e) {
			throw new SCEException("error.sce.unknown", e);
		}
	}
	...
	//Start of Nishtha
	public String getempbyNTID1(String ntid) {
		// TODO Auto-generated method stub
		String emplid = null;
		
			emplid = sceControl.getempbyNTID(ntid);
			System.out.println("emp id in managerimpl:"+emplid);
		/* catch (SQLException sqle) {
			sqle.printStackTrace();
			// log.error(LoggerHelper.getStackTrace(sqle));
			throw new SCEException("error.sce.database.communication", sqle);
